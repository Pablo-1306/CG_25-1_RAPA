...
int main()
{
	...
	camera = Camera(glm::vec3(0.0f, 5.0f, 7.0f), glm::vec3(0.0f, 1.0f, 0.0f), -90.0f, 0.0f, 0.3f, 0.5f);
	...
	dadoTexture = Texture("Textures/Dado10.tga");
	...
	letrero = Model();
	letrero.LoadModel("Models/letrero_.obj");
	Bus = Model();
	Bus.LoadModel("Models/bus.obj");
	...
	movCoche = 0.0f;	//posicion del coche
	movzCoche = 0.0f;	//posicion del coche
	movOffset = 0.3f;	//modificador de la posición
	movLetrero = 7.0f;	//posicion inicial del letrero
	movOffsetLet = 0.09f;	//modificador de la posición
	rotllanta = 0.0f;
	rotllantaOffset = 5.0f;
	rotletrero = 0.0f;
	rotletrero_2 = 0.0f;
	rotletreroOffset = 15.0f;
	avanza = true;
	baja = true;
	...
	while (!mainWindow.getShouldClose())
	{
		...
		deltaTime = now - lastTime;		//tiempo de cada ciclo del procesador
		deltaTime += (now - lastTime) / limitFPS;
		lastTime = now;

		//if (glfwGetTime() > 5) {
			if (movCoche > -90.0f && movzCoche == 0)
			{
				movCoche -= movOffset * deltaTime;
			} else if (movCoche <= -90 && movzCoche < 120){
				movzCoche += movOffset * deltaTime;
				c_2 = 2;
			}else if (movzCoche >= 120 && movCoche < 50) {
				movCoche += movOffset * deltaTime;
			}
			else if (movCoche >= 50 && movzCoche >= 0) {
				movzCoche -= movOffset * deltaTime;
			}
		...
		if (salto > 0) {
			if (baja) {
				if (movLetrero > 0.0f) {
					movLetrero -= movOffsetLet * deltaTime;
				}else{
			...
			if (c % 2 == 0) {
				...
				rotletrero = 7.5f;	//caso 7
				spotLights[1].SetPos(glm::vec3(5.0f, 10.0f, -9.8f));
				if (c == 9) {
					rotletrero_2 = 1.5;
				}
				else if (c == 3) {
		...
		//Dado 10 caras
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(5.0f - salto, 0.0f + movLetrero, -10.0f ));
		model = glm::scale(model, glm::vec3(2.0f, 2.0f, 2.0f));
		model = glm::rotate(model, -90 * toRadians, glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::rotate(model, -90 * rotletrero * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::rotate(model, -90 * rotletrero_2 * toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		////Instancia del coche 
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(30 + movCoche, 5.5f, -55.0f + movzCoche));
		modelaux = model;
		...
		//arco
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-10.0f, -0.8f, -15.0));
		modelaux = model;
		...
		//letrero
		model = modelaux;
		//+ movLetrero, rotletrero*
		model = glm::translate(model, glm::vec3(.2f, .8f , 0.0f));
		model = glm::scale(model, glm::vec3(0.5f, 0.5f, 0.5f));
